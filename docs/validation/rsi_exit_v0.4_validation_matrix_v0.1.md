...
| `TBD_PRIVATE_D` | 待定 | 待定 | 待定 | D 单峰快速回落 | `>=1` | `0` | 最多一个 forming chain；随后 `INVALIDATED`/`CLEARED` | 无第二可比结构峰 | 待补充 | 不得伪造背离链 |
...
| `TBD_PRIVATE_G` | 待定 | 待定 | 待定 | G 深度 reset | `>=2`（reset 前后） | 按样本 | 旧 warning 在阈值首次完成日 `CLEARED`；新周期使用新 id | 49 单日、49/50/49、40.1 | 待补充 | 覆盖 `<50` 三日与 `<=40` |
...
| 关键断言 | 第三日或 `<=40` 当日收盘后才 `CLEARED`；不得提前；新 forming 使用新 warning id |
| 数据范围 | reset 前至少 10 日，完成后至少至下一个结构峰确认 |
| 通过标准 | 正边界全部清除，负边界全部不清除；v0.3 divergence/risk 生命周期不被 warning 反向写入 |
| 失败标准 | 50 使用 `<=`、40 使用 `<`、或“即将 reset”提前触发 |
