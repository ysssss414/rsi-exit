...
基线数据存在一项规格内部冲突：5/14 前一交易日收盘 1049.20，5/20 high=1071、
close=1037；按 4.3 的冻结公式应为 `INTRADAY_POTENTIAL_RETEST`，而中际旭创章节
指定为 `NON_COMPARABLE_PEAK`。实现采用通用公式且不增加代码/日期特例；两者都不得
进入结构链或仓位系统。
