...
| `earliest_action_date` | 是 | 第一阶段等于 decision date，仅代表可显示；无交易含义 |
...
| `linked_formal_signal_id` | 否 | `ESCALATED` 时的正式信号引用 |
...
| `ACTIVE` | 客观动能重建或深度 reset 完成 | `CLEARED` | 终态；不是“从未有效” |
...
source.local_rsi_delta <= -divergence_rsi_tolerance   # v0.3 = -1.0
source.anchor_rsi_delta <= -anchor_rsi_tolerance     # v0.3 = -1.0
source.structural_eligible is False
source.position_eligible is False
source.pending_action_type is None
warning_event_id not in emitted_warning_event_ids
```

日期固定为：

```text
observation_date = decision_date = earliest_action_date = t
position_effect = NONE
```

这里的 `earliest_action_date` 只是“最早可向用户展示”的日期。第一阶段没有可执行仓位
动作，因此不允许把该字段传入 `SignalCapQueue`。若未来批准非空仓位效果，必须另行规定
`decision_date < earliest_action_date`，且至少为下一真实交易日。
...
source.signal_type == "DIVERGENCE_FORMING"
all(OPENED_threshold_conditions)
```
...
```text
ESCALATED > CLEARED > REFRESHED > INVALIDATED
```
...
active.decision_date < f.decision_date
```

满足时追加 `ESCALATED`，填入正式 canonical id/version 与 `linked_formal_signal_id`。
warning 不增加 count；count 只由正式背离原路径增加。`ESCALATED` 之后不再追加
`CLEARED`，历史链保留用于衡量预警提前量和误报率。

### 7.3 CLEARED：客观新事实终止风险观察

以下任一条件在决策日成立时追加 `CLEARED`：

1. 已确认、属于当前 latest lineage 的 `ANCHOR_RSI_BREAKOUT`；原因
   `MOMENTUM_ANCHOR_REBUILT`。
2. 从预警 observation date 之后的每日 RSI 首次满足 v0.3 冻结深度 reset：连续 3 个
   真实交易日 RSI 严格小于 50，或任一日 RSI 小于等于 40；原因
   `DEEP_RSI_RESET_COMPLETED`。
...
```text
observation_date <= decision_date <= earliest_action_date
```

第一阶段三者相等且无交易动作。终结事件的 observation date 是终结事实首次可见日，
不得写成原 warning 的日期。
...
| 深度 reset 当日同时有其他终结条件 | 按转换优先级处理，最多一个终结事件 |
...
以下问题留待 Phase 4 多样本结果后决定：

1. 是否值得新增“连续多个结构峰小幅 RSI 下降”的第二 warning type；若是，最小序列
   长度和累计下降阈值如何由正/负样本确定？
2. Phase 5 是否只显示方案 B 的建议 cap，还是继续保持纯审计？
3. 用户界面是否把 `REFRESHED` 折叠为一个活动 warning，审计 CSV 则保留全部版本？
4. 是否需要由 PeakDetector 显式携带稳定的 `forming_attempt_id -> confirmed canonical`
   映射，以替代本草案的峰日、anchor、last structural 和 chain 组合匹配？
5. 深度 reset 应显示为 `CLEARED` 还是单独的 `CYCLE_ENDED` 终态文案？数据语义相同，
   只影响报告表达。

以上问题均不得在本轮实现，也不得改变 v0.3.0 冻结语义。
